...
import { useGetLotteryGraphDataById } from 'state/bnblottery/hooks'
import { getGraphLotteries } from 'state/bnblottery/getLotteriesData'
...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    ...
    prizeInBusd = amountCollectedInCake.times(bnbBusdPrice)
  }
    ...
    return (
      <>
        {/* {prizeInBusd.isNaN() ? (
          ...
          <Heading scale="lg" lineHeight="1" color="text">
            {formatNumber(getBalanceNumber(prizeInBusd, 18), 3, 3)} BNB
          </Heading>
        )} */}
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            unit=" BNB"
            bold
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInCake, 18)}
            decimals={3}
          />
        )}
      </>
