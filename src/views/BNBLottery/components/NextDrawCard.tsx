...
import RewardBracketsNow from './RewardBracketsNow'
...
const NextDrawCard = () => {
  ...
  const prizeInBusd = amountCollectedInCake.plus(new BigNumber(828000000000000000000)).times(bnbBusdPrice)
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            textAlign={['center', null, null, 'left']}
            unit=" BNB"
            value={getBalanceNumber(amountCollectedInCake.plus(new BigNumber(828000000000000000000)))}
            decimals={3}
          />
        )}
      </>
  ...
  return (
    <StyledCard>
      <CardHeader p="24px 24px">
        <Flex justifyContent="space-between">
          <Heading mr="12px" color="secondary">{t('Next Draw')}</Heading>
      ...
      <CardBody>
        <Grid>
          <Flex justifyContent={['center', null, null, 'flex-start']}>
            <Heading color="text">{t('Prize Pot')}</Heading>
          </Flex>
          <Flex flexDirection="column" mb="18px">
            {getPrizeBalances()}
          </Flex>
          <Box display={['none', null, null, 'flex']}>
            <Heading color="text">{t('Your tickets')}</Heading>
          </Box>
          <Flex flexDirection={['column', null, null, 'row']} alignItems={['center', null, null, 'flex-start']}>
            {isLotteryOpen && (
              ...
              >
                {account && (
                  <Flex justifyContent={['center', null, null, 'flex-start']}>
                    <Text display="inline" color="textSubtle">{youHaveText} </Text>
                    ...
                    <Text display="inline" color="textSubtle"> {ticketsThisRoundText}</Text>
                  </Flex>
                )}
      ...
      <CardFooter p="0">
        {isExpanded && (
          <NextDrawWrapper>
            <RewardBracketsNow lotteryNodeData={currentRound}/>
          </NextDrawWrapper>
        )}
