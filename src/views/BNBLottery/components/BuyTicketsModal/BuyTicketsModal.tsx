...
import { useLottery } from 'state/bnblottery/hooks'
import { fetchUserTicketsAndLotteries } from 'state/bnblottery'
import useTheme from 'hooks/useTheme'
import useTokenBalance, { FetchStatus } from 'hooks/useTokenBalance'
import useConfirmTransaction from 'hooks/useConfirmTransaction'
import { useCake, useBNBLotteryContract } from 'hooks/useContract'
import { useCallWithGasPrice } from 'hooks/useCallWithGasPrice'
import useToast from 'hooks/useToast'
import ConnectWalletButton from 'components/ConnectWalletButton'
import { ToastDescriptionWithTx } from 'components/Toast'
import ConfirmButtons, { ButtonArrangement } from 'views/Profile/components/ConfirmButtons'
...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  const selectedCurrencyBalance = useCurrencyBalance(account ?? undefined, ETHER ?? undefined)
  ...
  const lotteryContract = useBNBLotteryContract()
  const cakeContract = useCake()
  const { toastSuccess } = useToast()
  // const { balance: userCake, fetchStatus } = useTokenBalance(getCakeAddress())
  const ethBalance = selectedCurrencyBalance?.toSignificant(6) ?? ''

  const userCake = useMemo(()=> (new BigNumber(ethBalance)).multipliedBy(BIG_TEN.pow(18)), [ethBalance])
  ...
  const hasFetchedBalance = selectedCurrencyBalance
  const userCakeDisplayBalance = getFullDisplayBalance(userCake, 18, 6)
  ...
  useEffect(() => {
    ...
    setTicketCostBeforeDiscount(costBeforeDiscount.gt(0) ? getFullDisplayBalance(costBeforeDiscount, 18) : '0')
    setTotalCost(costAfterDiscount.gt(0) ? getFullDisplayBalance(costAfterDiscount, 18) : '0')
    setDiscountValue(discountBeingApplied.gt(0) ? getFullDisplayBalance(discountBeingApplied, 18, 6) : '0')
  ...
  const { isConfirmed, isConfirming, handleConfirm } =
    useConfirmTransaction({
      onConfirm: () => {
        ...
        const bnbvalue = web3.utils.toWei(totalCost, "ether")
        return callWithGasPrice(lotteryContract, 'buyTickets', [currentLotteryId, ticketsForPurchase], { value: bnbvalue } )
      },
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCake) return t('Insufficient BNB balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInCake.times(new BigNumber(ticketsToBuy)), 18) : '0.00'} BNB`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInCake && getFullDisplayBalance(priceTicketInCake.times(ticketsToBuy || 0), 18)} BNB
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
        ...
        <Text mt="24px" fontSize="12px" color="textSubtle">
          {t(
            '"Buy Instantly" chooses random numbers, with no duplicates among your tickets. Prices are set before each round starts, equal to 0.1BNB at that time. Purchases are final.',
          )}
        </Text>
      </Flex>
    </StyledModal>
