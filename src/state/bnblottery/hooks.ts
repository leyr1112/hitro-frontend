...
export const useGetCurrentLotteryId = () => {
  return useSelector((state: State) => state.bnblottery.currentLotteryId)
}

export const useGetUserLotteriesGraphData = () => {
  return useSelector((state: State) => state.bnblottery.userLotteryData)
}
...
export const useGetLotteriesGraphData = () => {
  return useSelector((state: State) => state.bnblottery.lotteriesData)
}
...
export const useLottery = () => {
  const currentRound = useSelector((state: State) => state.bnblottery.currentRound)
  const processedCurrentRound = useProcessLotteryResponse(currentRound)

  const isTransitioning = useSelector((state: State) => state.bnblottery.isTransitioning)
  ...
  const maxNumberTicketsPerBuyOrClaimAsString = useSelector(
    (state: State) => state.bnblottery.maxNumberTicketsPerBuyOrClaim,
  )
